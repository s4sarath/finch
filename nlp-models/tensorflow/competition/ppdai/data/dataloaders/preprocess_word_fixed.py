...
class PreProcessor:
    def __init__(self):
        ...
        self.save_embedding_path = '../data/files_processed/embedding.npy'
        self.q2w_path = '../data/files_processed/q2w.pkl'

        self.tfrecord_train_path = '../data/tfrecords/word_train_fixed.tfrecord'
        self.tfrecord_val_path = '../data/tfrecords/word_val_fixed.tfrecord'
        self.tfrecord_test_path = '../data/tfrecords/word_test_fixed.tfrecord'
    ...
    def make_qid_to_word_dict(self):
        if os.path.isfile(self.q2w_path):
            tf.logging.info("Exists %s" % self.q2w_path)
        else:
            if not os.path.exists(os.path.dirname(self.q2w_path)):
                os.makedirs(os.path.dirname(self.q2w_path))
    ...
    def make_tfrecords(self):
        if (os.path.isfile(self.tfrecord_train_path)) and (os.path.isfile(self.tfrecord_val_path) and (os.path.isfile(self.tfrecord_test_path))):
            ...
            train_fn(train_csv, self.tfrecord_train_path, PAD_INT, q2w)
            train_fn(val_csv, self.tfrecord_val_path, PAD_INT, q2w)
